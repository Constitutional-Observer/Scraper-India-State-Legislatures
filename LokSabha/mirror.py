...
This script archives documents from the Parliament Digital Library (eparlib.sansad.in) to:
...
class EParlibMirror:
    ...
    def search_existing_ia_documents(self):
        ...
        try:
            ...
            for item in search_results:
                ...
                # Expected format: eparlib.nic.in.{doc_id}
                if identifier.startswith('eparlib.nic.in.'):
                    doc_id = identifier[len('eparlib.nic.in.'):]
    ...
    def upload_to_internet_archive(self, doc_id, metadata_path, pdf_paths, page_path):
        ...
        try:
            ...
            identifier = f"eparlib.nic.in.{doc_id}"
            ...
            source_url = pdf_urls[0] if pdf_urls else f'https://eparlib.sansad.in/handle/123456789/{doc_id}'
            
            ia_metadata = {
                'creator': 'Lok Sabha Secretariat',
                'source': f'https://eparlib.sansad.in/handle/123456789/{doc_id}',
                'mediatype': 'texts',
                'language': ['English', 'Hindi'],
                'licenseurl': 'http://creativecommons.org/licenses/publicdomain/',
                'subject': ['Parliament of India', 'Lok Sabha'],
            }
            ...
            if metadata_path and metadata_path.exists():
                ...
                if 'title' in doc_metadata:
                    ia_metadata['title'] = f'{doc_metadata["title"]} ({doc_metadata["date"]})'
                if 'date' in doc_metadata:
                    ia_metadata['date'] = doc_metadata['date']
                
                # Add all fields as custom metadata
                for key, value in doc_metadata.items():
                    ia_metadata[f"eparlib_{key.lower()}"] = value

            ia_metadata['description'] = f"'{ia_metadata['title']}' from the Parliament Digital Library"
            ia_metadata['eparlib_document_url'] = source_url
    ...
    def run(self):
        ...
        logger.info("Starting ePARLIB Mirror Tool")
