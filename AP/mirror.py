...
This script archives documents from the Andhra Pradesh Legislature to:
...
BASE_URL = "https://www.aplegislature.org"
ARCHIVES_URL = f"{BASE_URL}/web/aplegislature/archives-tree"
DOCUMENT_BASE_URL = "https://sessions.aplegislature.org/preview.do"

class APLegislatureMirror:
    def __init__(self):
    ...
    def search_existing_ia_documents(self):
        """Search Internet Archive for existing AP Legislature documents"""
        try:
            logger.info("Searching Internet Archive for existing documents...")
            
            # Search for documents with our creator
            search_query = 'creator:"Andhra Pradesh State Legislature"'
            ...
            for item in search_results:
                ...
                # Expected format: aplegislature.{house}.{term}.{session}.{sitting}.{filename}
                if identifier.startswith('aplegislature.'):
    ...
    def parse_archives_tree(self, tree_filepath):
        ...
        try:
            ...
            # Find the main treeview
            main_treeview = soup.find('li', class_='treeview')
            if not main_treeview:
                logger.error("No main treeview found in archives page")
                return []
            
            # Parse Assembly and Council sections
            houses_ul = main_treeview.find('ul', class_='treeview-menu')
            houses = houses_ul.find_all('li', recursive=False)
            for house_li in houses:
                house_link = house_li.find('a')
                if not house_link:
                    continue
                
                house_text = house_link.get_text(strip=True)
                ...
                # Parse terms within each house
                terms = house_li.find_all('ul', class_='treeview-menu', recursive=False)
                for term_ul in terms:
                    term_items = term_ul.find_all('li', recursive=False)
                    for term_li in term_items:
                        term_link = term_li.find('a')
                        if not term_link:
                            continue
                        
                        term_text = term_link.get_text(strip=True)
                        
                        # Parse sessions within each term
                        sessions = term_li.find_all('ul', class_='treeview-menu', recursive=False)
                        for session_ul in sessions:
                            session_items = session_ul.find_all('li', recursive=False)
                            for session_li in session_items:
                                session_link = session_li.find('a')
                                if not session_link:
                                    continue
                                
                                session_text = session_link.get_text(strip=True)
                                
                                # Parse sittings within each session
                                sittings = session_li.find_all('ul', class_='treeview-menu', recursive=False)
                                for sitting_ul in sittings:
                                    sitting_items = sitting_ul.find_all('li', recursive=False)
                                    for sitting_li in sitting_items:
                                        sitting_link = sitting_li.find('a')
                                        if not sitting_link:
                                            continue
                                        
                                        sitting_text = sitting_link.get_text(strip=True)
                                        
                                        # Parse individual document days within each sitting
                                        days = sitting_li.find_all('ul', class_='treeview-menu', recursive=False)
                                        for day_ul in days:
                                            ...
                                            for day_li in day_items:
                                                ...
                                                if filename_match:
                                                    ...
                                                    document = {
                                                        ...
                                                        'url': href,
                                                        'identifier': f"{house_name.lower()}.{filename.replace('.pdf', '').replace('-', '.').replace(' ', '_')}"
                                                    }
                                                    documents.append(document)
            
            logger.info(f"Found {len(documents)} documents in archives tree")
            return documents
    ...
    def upload_to_internet_archive(self, document, metadata_path, pdf_path):
        ...
        try:
            ...
            identifier = f"aplegislature.{document['identifier']}"
            
            # Prepare metadata for Internet Archive
            ia_metadata = {
                'creator': 'Andhra Pradesh State Legislature',
                'source': document['url'],
                'mediatype': 'texts',
                'language': ['English', 'Telugu'],
                'licenseurl': 'http://creativecommons.org/licenses/publicdomain/',
                'subject': ['Andhra Pradesh State Legislature'],
            ...
            if metadata_path and metadata_path.exists():
                ...
                ia_metadata['description'] = f"Andhra Pradesh State Legislature {document['house']} proceedings - {document['term']}, {document['session']}, {document['sitting']}, {document['day']}"
                ...
                for key, value in doc_metadata.items():
                    if key not in ['url', 'source_url', 'processing_date']:
                        ia_metadata[f"ap_legislature_{key}"] = value
    ...
    def process_all_documents(self):
        ...
        logger.info("Starting to process all AP Legislature documents")
    ...
    def run(self):
        ...
        logger.info("Starting Andhra Pradesh Legislature Mirror Tool")
